...
// Lấy giá trị số liệu của các bảng bác sĩ, lịch khám bệnh, hồ sơ bệnh, số thuốc
router.get('/', function(req, res) {
    db.getAllDoc(function(err, result) {
        db.getallappointment(function(err, result1) {
            db.getAllPatients(function(err, result2) {
                db.getallmed(function(err, result3) {
                    var total_doc = result.length;
                    var appointment = result1.length;
                    var patients = result2.length;
                    var med = result3.length;
                    res.render('home.ejs', {
                        doc: total_doc,
                        doclist: result,
...
router.post('/add_departments', function(req, res) {
    ...
    db.add_dept(tenchuyenkhoa, mota, function(err, result) {
...
router.post('/edit_department/:id', function(req, res) {
    db.edit_dept(req.params.id, req.body.tenchuyenkhoa, req.body.mota, function(err, result) {
        res.redirect('/home/departments');
    });
});

router.post('/departments/search', function(req, res) {
    var key = req.body.search;
    db.searchdept(key, function(err, result) {
        console.log(result);
        res.render('departments.ejs', { list: result });
    });
});
